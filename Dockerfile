...
ENV GRAAL_PKG graalvm-ce-java8-linux-amd64-$GRAAL_VERSION
WORKDIR /app
...
RUN for name in $(ls /tmp/certs); do /app/graalvm-ce-java8-$GRAAL_VERSION/jre/bin/keytool -importcert -v -keystore /app/graalvm-ce-java8-$GRAAL_VERSION/jre/lib/security/cacerts -storepass changeit -file /tmp/certs/$name -noprompt -alias $name ; done && \
    rm -rf /tmp/certs

RUN rm -rf /app/graalvm-ce-java8-$GRAAL_VERSION/jre/bin/polyglot
RUN rm -rf /app/graalvm-ce-java8-$GRAAL_VERSION/jre/languages
RUN rm -rf /app/graalvm-ce-java8-$GRAAL_VERSION/jre/lib/polyglot
...
COPY --from=build-env /app/graalvm-ce-java8-$GRAAL_VERSION/jre /usr/lib/jvm/graalvm-$GRAAL_VERSION
